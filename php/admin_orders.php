<?php
// php/admin_orders.php  —  Admin: view & update orders
require 'config.php';
require_admin();
header('Content-Type: application/json');

// ---- CSRF (state-changing requests only) --------------------
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $csrf = $_POST['csrf_token'] ?? '';
    if (!verify_csrf($csrf, 'admin_action')) {
        http_response_code(403);
        json_response(false, null, 'Invalid or expired form token. Please refresh and try again.');
        exit();
    }
}

$action = clean($_POST['action'] ?? $_GET['action'] ?? '');

switch ($action) {

    case 'read':
        $status = clean($_GET['status'] ?? '');
        if ($status && in_array($status, ['Pending','Preparing','Completed','Cancelled'])) {
            $stmt = $pdo->prepare(
                'SELECT * FROM orders WHERE status = ? ORDER BY order_date DESC'
            );
            $stmt->execute([$status]);
        } else {
            $stmt = $pdo->query('SELECT * FROM orders ORDER BY order_date DESC');
        }
        json_response(true, $stmt->fetchAll());
        break;

    case 'update_status':
        $id     = (int)($_POST['id']     ?? 0);
        $status = clean($_POST['status'] ?? '');
        if (!$id || !in_array($status, ['Pending','Preparing','Completed','Cancelled'])) {
            json_response(false, null, 'Valid ID and status required.');
            exit();
        }
        $stmt = $pdo->prepare('UPDATE orders SET status = ? WHERE id = ?');
        $ok   = $stmt->execute([$status, $id]);
        $ok
            ? json_response(true, null, "Order marked as {$status}.")
            : json_response(false, null, 'Update failed.');
        break;

    // Stats used by dashboard
    case 'stats':
        $row = $pdo->query(
            "SELECT
                COUNT(*)  AS total,
                SUM(status='Pending')   AS pending,
                SUM(status='Preparing') AS preparing,
                SUM(status='Completed') AS completed,
                SUM(status='Cancelled') AS cancelled,
                COALESCE(SUM(CASE WHEN status='Completed' THEN total_amount END), 0) AS total_revenue
             FROM orders"
        )->fetch();
        json_response(true, $row);
        break;

    default:
        http_response_code(400);
        json_response(false, null, 'Invalid action.');
}
